...
function QuestionItem({ question, index, onChange }: { question: BulkInputQuestion, index: number, onChange: (action: 'update' | 'delete', index: number, question?: BulkInputQuestion) => void }) {
    ...
    function onUpdateTemplate(field: 'question' | 'answer' | 'flagged', value: string | boolean) {
        onChange('update', index, {
            ...question,
            template: {
                ...question.template,
    ...
    return (
        <div className="QUESTION ITEM bg-lightgray rounded-3xl py-4 px-8 flex flex-col gap-6" key={index}>
            <div className="TOP BAR flex flex-row gap-8 justify-between">
                ...
                <button onClick={() => onChange('delete', index)}><TrashIcon className="w-6 h-6" /></button>
                <button onClick={() => onChange('update', index, {...question, flagged: !question.flagged})}><FlagIcon className={`w-6 h-6  ${question.flagged ? 'text-red-500 hover:text-red-600' : 'hover:text-gray-500'}`} /></button>
                </div>
            </div>
            {tab === 'preview' ? (
...
export default function SmartCategory() {
    ...
    const [publicCategory, _setPublicCategory] = useState(false);
    ...
    async function onPostCategory() {
        setIsPosting(true);
        const response = await handlePostCategoryQuestions(title, description, tags, publicCategory, inputQuestions.map(q => q.template));
        if(response.success) {
