...
function QuestionItem({ question, index, onChange, onRequestSampleRefresh }: { 
    ...
    // Instead of individual debounced calls, request a bulk refresh
    ...
    useEffect(() => {
        // Only request refresh if the question has variables that need sampling
...
export default function EditSet() {
    ...
    const debouncedBulkSampleFetch = useDebouncedCallback(
        async () => {
            ...
            try {
                // Get all questions that need sample generation (have variables)
                ...
                // Prepare templates for bulk API
                ...
                if (response.success && response.samples) {
                    // Update questions with new samples
                    setInputQuestions(prevQuestions => {
                        ...
                        for (let i = 0; i < newQuestions.length; i++) {
                            ...
                            // Check if this question needed a sample
                            if ((question.template.rvs.length > 0 || question.template.pvs.length > 0) && 
                                question.template.question.trim()) {
        ...
        1500 // Slightly longer debounce for bulk operations
    );
    ...
    // Load existing category and questions data
    useEffect(() => {
        if (categoryId) {
            loadCategoryData();
        }
    ...
    async function loadCategoryData() {
        ...
        try {
            ...
            // Fetch question list - this returns GeneratedQuestionType[] objects, not just IDs
            const questionList = await fetchQuestionList(categoryId);
            
            // Convert existing questions to BulkInputQuestion format
            const existingQuestions: BulkInputQuestion[] = [];
            
            // questionList is an array of GeneratedQuestionType objects with id field
            for (const questionObj of questionList) {
                try {
                    // Use the question ID to fetch detailed template data
            ...
            // Trigger initial bulk sample fetch for questions with variables
            if (existingQuestions.some(q => 
                (q.template.rvs.length > 0 || q.template.pvs.length > 0) && 
                q.template.question.trim()
            )) {
                // Use setTimeout to ensure state is updated before triggering sample fetch
                setTimeout(() => {
    ...
    async function onSaveSet() {
        ...
        try {
            // For now, we'll use the existing handlePostQuestions API
            // In a real implementation, you'd want a separate update API
            const result = await handlePostQuestions(inputQuestions.map(q => q.template), categoryId);
            
            if(result.success) {
                setMessage('Set saved successfully');
                // Optionally navigate back to the category
                setTimeout(() => {
    ...
    <div className="flex flex-col gap-4 mx-4 lg:mx-16 md:mx-12 my-12">
        ...
        <div className="flex flex-row justify-between items-center">
            <div className="flex flex-col gap-2">
                <h1 className="font-semibold text-5xl">Edit Set</h1>
                {category && (
            ...
            <div>
                <button 
                    disabled={inputQuestions.length === 0 || isSaving} 
                    onClick={onSaveSet} 
                    className={`bg-darkgray text-white px-6 py-2 rounded-lg font-medium hover:scale-105 transition-transform ${inputQuestions.length === 0 || isSaving ? 'opacity-50' : ''}`}
                >
        ...
        <div className="flex flex-col gap-4">
            ...
            <div className="flex justify-end">
                <button 
                    disabled={inputQuestions.length === 0 || isSaving} 
                    onClick={onSaveSet} 
                    className={`bg-darkgray text-white px-8 py-4 rounded-xl text-lg font-medium hover:scale-105 transition-transform ${inputQuestions.length === 0 || isSaving ? 'opacity-50' : ''}`}
                >
                    {isSaving ? 'Saving...' : 'Save'}
                </button>
            </div>
